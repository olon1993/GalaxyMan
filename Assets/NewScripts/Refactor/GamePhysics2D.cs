...
public class GamePhysics2D : RaycastBox2D
{
	...
	[SerializeField] protected float _maxJumpHold = 1.2f;
	[SerializeField] protected float _jumpVelocity = 3f;

	// Collisions
	protected CollisionInfo _collisions;

	// Climbing
	[SerializeField] protected float _maxSlopeAngle = 60;
	...
	protected virtual void Update() {
		...
		if (_collisions.Above || _collisions.Below) {
			if (_collisions.SlidingDownMaxSlope) {
				_velocity.y += _collisions.SlopeNormal.y * -_gravity * Time.deltaTime;
			} else {
				_velocity.y = 0;
			}
		}
	...
	protected virtual void CalculateVelocity() {
		...
		_velocity.x = Mathf.SmoothDamp(_velocity.x, targetVelocityX, ref _velocityXSmoothing, _smoothTime);
